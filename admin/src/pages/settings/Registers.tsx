...
export const Registers = () => {
  ...
  const handleSave = () => {
    if (newRegister.name.trim()) {
      setRegisters([...registers, { ...newRegister, id: Date.now() }]);
      setNewRegister({ name: '', branch: 'Main Branch', status: true });
    ...
    <div className="flex-1 bg-gray-50/50 min-h-screen">
      <div className="p-8">
        <div className="mb-8">
          ...
          <p className="text-sm text-gray-500 mt-1">Manage POS registers across your branches.</p>
        </div>

        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h2 className="text-lg font-bold text-gray-900">Register List</h2>
              <p className="text-sm text-gray-500">Displaying {filtered.length} register{filtered.length !== 1 && 's'}</p>
            </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-gray-100">
                  <th className="py-4 px-4 text-sm font-semibold text-gray-900">Name</th>
                  <th className="py-4 px-4 text-sm font-semibold text-gray-900">Branch</th>
              ...
              <tbody>
                {filtered.map((reg) => (
                  <tr key={reg.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                    <td className="py-4 px-4 text-sm text-gray-600">{reg.name}</td>
                    <td className="py-4 px-4 text-sm text-gray-600">{reg.branch}</td>
                    <td className="py-4 px-4">
                      <label className="relative inline-flex items-center cursor-pointer">
                        <input 
                          ...
                          checked={reg.status}
                          onChange={() => {
                            setRegisters(registers.map(r => r.id === reg.id ? { ...r, status: !r.status } : r));
                          }}
                        />
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Add New Register">
        <div className="space-y-4">
          ...
          <div>
            ...
            <select 
              ...
              value={newRegister.branch}
              onChange={(e) => setNewRegister({ ...newRegister, branch: e.target.value })}
            >
          ...
          <div className="flex items-center justify-between">
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input 
                type="checkbox" 
                className="sr-only peer" 
                checked={newRegister.status}
                onChange={(e) => setNewRegister({ ...newRegister, status: e.target.checked })}
              />
              <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
            </label>
          </div>
          <div className="pt-4 flex justify-end gap-3">
            ...
            <button onClick={handleSave} className="px-4 py-2 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition-colors">Save Register</button>
          </div>
        </div>
      </Modal>
    </div>
