...
export const Categories = () => {
  ...
  useEffect(() => {
    const cats = productsData.categories.map((c: any) => ({
      id: c.id.toString(),
      name: c.name,
      children: []
    }));
      ...
      setCategories(
        categories.map(cat =>
          cat.id === newCategory.parentId
            ? {
                ...cat,
                children: [...(cat.children || []), { id: `${cat.id}-${Date.now()}`, name: newCategory.name }]
              }
            : cat
        )
      );
  ...
  const deleteCategory = (id: string, isParent: boolean) => {
    ...
    if (isParent) {
      ...
      setCategories(categories.map(c => ({ ...c, children: c.children?.filter(ch => ch.id !== id) })));
    }
  };
  ...
  return (
    <div className="p-8 max-w-[1600px] mx-auto bg-white min-h-screen">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex-1 flex items-center justify-between px-4 py-3 border border-gray-200 rounded-lg bg-white shadow-sm hover:border-emerald-200 transition-all">
                  ...
                  <div className="opacity-0 group-hover:opacity-100 flex items-center gap-2 transition-opacity">
                    ...
                    <button
                      onClick={() => handleOpenEdit(category.id, category.name, true)}
                      className="p-1 text-gray-400 hover:text-emerald-600"
                    >
              ...
              {expanded[category.id] && category.children && category.children.length > 0 && (
                <div className="ml-[10px] pl-[26px] border-l border-emerald-100 mt-2 space-y-2">
                  {category.children.map(child => (
                    <div
                      key={child.id}
                      className="flex items-center justify-between px-4 py-3 border border-gray-100 rounded-lg bg-gray-50/30 shadow-sm relative group/child"
                    >
                      ...
                      <div className="opacity-0 group-hover/child:opacity-100 flex items-center gap-2">
                        <button
                          onClick={() => handleOpenEdit(child.id, child.name, false)}
                          className="p-1 text-gray-400 hover:text-emerald-600"
                        >
                          <Edit className="w-4 h-4" />
                        </button>
      ...
      <Modal
        ...
        title={editingItem ? 'Edit Category' : newCategory.parentId ? 'Add Sub-category' : 'Add Category'}
      >
        <form onSubmit={handleAddCategory} className="space-y-4">
          <input
            ...
            className="w-full px-4 py-2 border rounded-lg focus:border-emerald-500 outline-none"
            required
            autoFocus
          />
          <div className="flex justify-end gap-3 pt-4">
            <button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 text-sm text-gray-500">
              Cancel
            </button>
